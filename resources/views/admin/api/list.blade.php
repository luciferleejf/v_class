...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.api') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.api.list')}}</span>
            </div>
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="7%"> {{ trans('labels.api.type') }} </th>
                          <th width="7%"> {{ trans('labels.api.name') }} </th>
                          <th> {{ trans('labels.api.url') }} </th>
                          <th> {{ trans('labels.api.parms') }} </th>
                          <th width="10%"> {{ trans('labels.api.parmsDetail') }} </th>
                          <th width="12%"> {{ trans('labels.api.jason') }} </th>
                          <th width="20%"> {{ trans('labels.api.jasonDetail') }} </th>
                          <th width="7%"> {{ trans('labels.api.requestNum') }} </th>
                        </tr>
...
<script type="text/javascript" src="{{asset('backend/js/api/api-list.js')}}"></script>
